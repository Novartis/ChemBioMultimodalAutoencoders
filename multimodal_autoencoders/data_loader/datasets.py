...
class PairedDataset(BaseDataset):
    def __init__(self, data_dict: Dict[str, np.array], cluster_labels: Union[np.array, None] = None):
    ...
    def _set_length(self, data_dict: Dict[str, np.array]):
        """
        """
        len_list = [len(v) for v in data_dict.values()]
        return len_list[0]
...
class UnpairedDataset(BaseDataset):
    def __init__(
        self, data_dict: Dict[str, np.array], cluster_labels:Union[Dict[str, np.array], None]  = None):
        # parent class should take care of data needed for init
        super().__init__()
    ...
    def _set_cluster_labels(self, cluster_labels: Union[Dict[str, np.array], None]  = None):
        """
        """
        cluster_dict = {}
        # check if cluster info is available
