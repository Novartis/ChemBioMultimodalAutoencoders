...
from multimodal_autoencoders.model.metric import *
import logging
import os
import copy
import heapq
...
class JointTrainer():
    ...
    def _set_recon_weight_dict(self, recon_weight: Union[float, Dict[str, float]]) -> Dict[str, float]:
        ...
        recon_weight_dict = {}
        if isinstance(recon_weight, int):
    ...
    def train(
        ...
        for epoch in range(self.max_epochs):
            ...
            es_result = self._early_stopping(
                ckp_heap, patience, loss_diff, min_value, min_val_breaches, epoch)
    ...
    def forward(self, model_key: str, X: np.array, batch_size: int = 256, use_gpu: bool = False) -> tuple:
        ...
        for idx, batch in enumerate(dataloader):
            ...
            with torch.no_grad():
                recon, latents, _, _ = model.forward(
                    self._batchtensor_to_device(data))
    ...
    def _update_meter_dict(self, meter_dict: Dict[str, object], meter_name, value: torch.Tensor):
